...
//    각 컴퓨터들과 회선은 그 성능이 차이가 날 수 있다. 따라서 각각의 직접 연결되어 있는 회선을 이용해서 통신을 하는데 걸리는 시간이 서로 다를 수 있다. 심지어는 직접 연결되어 있는 회선이 오히려 더 느려서, 다른 컴퓨터를 통해서 통신을 하는 것이 더 유리할 수도 있다. 직접 연결되어 있는 회선을 사용할 경우에는 그 회선을 이용해서 통신을 하는 데 드는 시간만큼이 들게 된다.
...
//    관리자는 컴퓨터에 보안 시스템을 설치하려 하였는데, 버전 문제로 보안 시스템을 한 대의 슈퍼컴퓨터에만 설치할 수 있었다. 한 컴퓨터가 공격을 받게 되면,
//    네트워크를 통해 슈퍼컴퓨터에 이 사실이 전달이 되고, 그러면 슈퍼컴퓨터에서는 네트워크를 이용해서 보안 패킷을 전송하는 방식을 사용하기로 하였다
//    . 준비를 마친 뒤, 관리자는 다시 네트워크를 복구하기로 하였다. 이때, 다음의 조건들이 만족되어야 한다.
//
//    해커가 다시 공격을 할 우려가 있기 때문에, 최소 개수의 회선만을 복구해야 한다.
//    물론, 그렇다면 아무 회선도 복구하지 않으면 되겠지만, 이럴 경우 네트워크의 사용에 지장이 생기게 된다. 따라서 네트워크를 복구한 후에 서로 다른 두 컴퓨터 간에 통신이 가능하도록 복구해야 한다.
//    네트워크를 복구해서 통신이 가능하도록 만드는 것도 중요하지만,
...
//    따라서 슈퍼컴퓨터가 다른 컴퓨터들과 통신하는데 걸리는 최소 시간이, 원래의 네트워크에서 통신하는데 걸리는 최소 시간보다 커져서는 안 된다.
    // 슈퍼 컴퓨터가 1번으로 고정되어있기 떄문에 prim 알고리즘을 사용하면 안됨
...
//    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < M; i++) {
            ...
            arr.get(v).add(new Node(v, u, w));
            //무방향 그래프 start end weight 값
        }
        dist = new int[N + 1];
        Arrays.fill(dist, INF);
        visited = new boolean[N + 1];
        answer = new ArrayList<>(); //정답 배열 생성
        dijkstra(1);
        //System.out.println(answer);
        System.out.println(answer.size());
        for (int i = 0; i < answer.size(); i++) {
            System.out.println(answer.get(i).startNode + " " + answer.get(i).endNode);
        }
    ...
    public static void dijkstra(int start) {
        ...
        while (!pq.isEmpty()) { // start node 가 부모 노드
            Node current = pq.poll();
            if (visited[current.endNode]) continue;
            visited[current.endNode] = true;
            if (current.startNode != current.endNode) answer.add(current);

            for (Node next : arr.get(current.endNode)) {
                if (dist[next.endNode] > dist[current.endNode] + next.weight) {
                    dist[next.endNode] = dist[current.endNode] + next.weight;
                    pq.offer(new Node(next.startNode, next.endNode, dist[next.endNode]));
                }
            }
        }
    }
